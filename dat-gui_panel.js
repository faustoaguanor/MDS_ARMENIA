...
(function () {
  ...
  _this.init = function (setupDefaultItems, params) {
    ...
    if (setupDefaultItems) {
      this.layersFolder = panel.addFolder('Capas');
  ...
  _this.initLayersFolder = function (scene) {
    ...
    for (var layerId in mapLayers) {
      ...
      folder.add(params.lyr[layerId], 'o').min(0).max(1).name('Opacidad').onChange(opacityChanged);
  ...
  _this.addHelpButton = function () {
    panel.add(this.parameters, 'i').name('Ayuda');
  };
})();
